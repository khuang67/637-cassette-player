  ...
  return (
    <div className="absolute left-[85.51px] size-[109.178px] top-[-11.83px]">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 110 110">
        <g id="SunGraphic">
          <path d={svgPaths.pdfcc980} fill="var(--fill-0, #EDA35F)" id="Subtract" />
        </g>
      </svg>
    </div>
  );
...
function Icon() {
  return (
    <div className="bg-[#eda35f] box-border content-stretch flex gap-[0.808px] items-start p-[0.647px] relative shrink-0">
...
function Text({ songName }: { songName?: string }) {
  return (
    <div className="box-border content-stretch flex gap-[0.808px] h-[13.743px] items-start overflow-clip px-0 py-[1.293px] relative shrink-0">
      <p className="font-['Agrandir:Text_Bold',sans-serif] leading-[0.93] not-italic relative shrink-0 text-[15.669px] text-nowrap text-white tracking-[-0.9401px] uppercase whitespace-pre">{songName || 'mudo'}</p>
    </div>
  );
}
...
function Cover({ songName }: { songName?: string }) {
  return (
    <div className="absolute bg-[#261d1e] h-[81.311px] left-[11.83px] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[0px] mask-size-[182.854px_81.311px] overflow-clip top-[10.69px] w-[182.854px]" style={{ maskImage: `url('${imgCover}')` }}>
      <SunGraphic />
...
export default function Cassette({ songName }: CassetteProps) {
  return (
    <div className="relative size-full">
      <div className="absolute bg-[#fce6d2] h-[125.211px] left-[0.57px] rounded-[8.398px] top-[0.57px] w-[204.995px]">
      ...
      <div className="absolute inset-[32.18%_20.56%_40.94%_20.74%]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 122 34">
          <path d={svgPaths.p2f910c00} fill="var(--fill-0, #FCE6D2)" id="BG" stroke="var(--stroke-0, #202020)" strokeWidth="0.572612" />
        </svg>
      </div>
      <PencilThingy />
      <PencilThingy1 />
